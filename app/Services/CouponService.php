...
class CouponService
{
    ...
    public function couponChecking(CouponCheckRequest $request)
    {
        try {
            ...
            if ($coupon) {
                // Check if the coupon is user-specific and matches the authenticated user
                if ($coupon->user_id && $coupon->user_id !== auth()->user()->id) {
                    throw new Exception(trans('all.message.coupon_not_applicable'), 422);
                }

                if ($coupon->minimum_order > $request->total) {
                    throw new Exception(trans('all.message.you_have_to_buy_more_to_use_this_coupon') . AppLibrary::convertAmountFormat($coupon->minimum_order), 422);
                }
