    ...
    public function store(MessageRequest $request)
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                //     $existingMessage = Message::where('id', $request->message_id)
                //         ->where('user_id', $user->id)
                //         ->where('branch_id', 1)
                //         ->first();

                //     if (!$existingMessage) {
                //         $request->merge(['message_id' => null]); // Set message_id to null if not valid
                //     }
                // }
                if ($request->message_id) {
                    ...
                    $this->message = Message::create(
                        ...
                        ) + ['user_id' => $user->id, 'branch_id' => 1] // Ensure branch_id is set to 1 for new messages
                    );
                    $messageHistory = MessageHistory::create(
